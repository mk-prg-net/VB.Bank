...
-- --------------------------------------------------
-- Date Created: 06/29/2016 16:30:20
-- Generated from EDMX file: C:\Users\Schulung\Documents\Visual Studio 2013\Projects\Bank\LinkDB\KeplerDB.edmx
-- --------------------------------------------------
...
CREATE TABLE [dbo].[Sterne_Planeten_MondeTab] (
    ...
    [HimmelskoerperID] int  NOT NULL,
    [Leuchtkraft_in_Lsonne] float  NOT NULL
);
GO
...
CREATE TABLE [dbo].[RaumschiffAufgabenTab] (
    [AufgabenTab_ID] int  NOT NULL,
    [RaumschiffeTab_HimmelskoerperID] int  NOT NULL
);
GO
...
-- Creating primary key on [AufgabenTab_ID], [RaumschiffeTab_HimmelskoerperID] in table 'RaumschiffAufgabenTab'
ALTER TABLE [dbo].[RaumschiffAufgabenTab]
ADD CONSTRAINT [PK_RaumschiffAufgabenTab]
    PRIMARY KEY CLUSTERED ([AufgabenTab_ID], [RaumschiffeTab_HimmelskoerperID] ASC);
GO
...
-- Creating foreign key on [AufgabenTab_ID] in table 'RaumschiffAufgabenTab'
ALTER TABLE [dbo].[RaumschiffAufgabenTab]
ADD CONSTRAINT [FK_RaumschiffAufgabenTab_AufgabenTab]
    FOREIGN KEY ([AufgabenTab_ID])
    REFERENCES [dbo].[AufgabenTab]
        ([ID])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO

-- Creating foreign key on [RaumschiffeTab_HimmelskoerperID] in table 'RaumschiffAufgabenTab'
ALTER TABLE [dbo].[RaumschiffAufgabenTab]
ADD CONSTRAINT [FK_RaumschiffAufgabenTab_RaumschiffeTab]
    FOREIGN KEY ([RaumschiffeTab_HimmelskoerperID])
...
ON [dbo].[RaumschiffAufgabenTab]
    ([RaumschiffeTab_HimmelskoerperID]);
GO
